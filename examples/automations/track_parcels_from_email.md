# Automatic parcel tracking from e-mail (IMAP → Swiss Post)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Swiss Post tracking code, and register it with `swiss_post.track_parcel` — fully automatic, no extra custom component required.

Swiss Post is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `swiss_post.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ swiss_post.track_parcel
                                     │
...
- This integration, with the `swiss_post.track_parcel` action available (field `tracking_code`).
...
Two patterns run over each mail:

```
(?<![0-9])(?:99[0-9. ]{16,22})(?![0-9])          # domestic, separators included
(?<![0-9A-Za-z])[A-Z]{2}[0-9]{9}CH(?![0-9A-Za-z])  # international S10
```

Two shapes are covered: the **18-digit domestic number** — matched with its printed separators (`99.60 1234.5678 9012 34`) and then stripped down to digits — and the **international S10 form** (`RR123456789CH`). The integration itself accepts any 8–20 digit run and any S10 code, but matching *that* broadly against e-mail text would hit every order and invoice number in the mail, so the example anchors on the `99` prefix Swiss Post parcel numbers start with. **If your parcels arrive under a different pattern, widen the first `regex_findall`**; the AI fallback will surface unknown formats in the meantime.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\b99…')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9A-Za-z])` lookarounds instead of `\b`, `[0-9]` instead of `\d`. Copy that style if you widen the pattern.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@swiss_postshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: 99.60 1234.5678 9012 34","initial":true,"folder":"INBOX","username":"test"}'
```

Then `swiss_post.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
